...
SECRET_KEY = "faang-80-lpa-secret-key"
ALGORITHM = "HS256"
...
# -------------------------------------------------------------------
async def trigger_slack_webhook(vulnerability_id: str, patch: dict):
    """
    Simulates sending an Incident Response Webhook to a Slack/Teams channel.
    """
    payload = {
        "text": f"🚨 *CRITICAL SECURITY INCIDENT RESOLVED* 🚨\n"
                f"*Vulnerability ID:* {vulnerability_id}\n"
                f"*Action Taken:* Auto-Patched via GenAI RAG Engine.\n"
                f"*Patch Details:* ```json\n{json.dumps(patch, indent=2)}\n```"
    }
    # In production: httpx.post("https://hooks.slack.com/services/T00000000/B00000000/XXXXXXXXXXXXXXXXXXXXXXXX", json=payload)
...
async def scan_infrastructure(payload: dict):
    ...
    for vuln in vulns:
        ...
        # 4. Trigger Slack Webhook
        await manager.broadcast({"type": "status", "message": f"Sending Incident Report to Corporate Slack Channel..."})
        slack_payload = await trigger_slack_webhook(vuln["node_id"], patch_result["patch"])
        await manager.broadcast({
            "type": "slack_alert",
            "message": "Slack Alert Successfully Dispatched."
        })
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    if form_data.username == "admin" and form_data.password == "admin123":
        payload = {"sub": form_data.username, "exp": time.time() + 3600}
...
async def chat_with_rag_agent(chat: ChatMessage):
    ...
    if "s3" in user_msg or "bucket" in user_msg:
        ...
        response_text = "Hello Administrator. I am actively monitoring the AWS and Azure infrastructure graph. How can I assist you with your Zero-Trust posture today?"
    elif "vulnerabilit" in user_msg or "found" in user_msg or "status" in user_msg:
        response_text = "The Graph ML engine is continuously scanning. Please refer to the middle Orchestration Log to see the live Zero-Trust patches being deployed by the GenAI Agent."
    else:
        response_text = "I am the Zero-Trust RAG Agent. My context is restricted to Cloud Security. Please ask me about AWS S3 policies, Azure VM rules, or Graph ML toxic combinations."
